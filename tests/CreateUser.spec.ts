...
async function createNewUser(
  ...
  const fullName = options?.fullName ?? 'Automation Test User';

  const email = options?.email ?? `${username}@test.com`;

  const password = options?.password ?? 'Test@12345';

  const partitions = options?.partitions ?? ['NAM'];

  const defaultPartition = options?.defaultPartition ?? partitions[0];

  await page.locator('#addEditUser_form').waitFor({ state: 'visible' });

  await page.locator('#addEditUser_form_userName').fill(username);

  await page.locator('#addEditUser_form_fullName').fill(fullName);

  await page.locator('#addEditUser_form_email').fill(email);

  await page.locator('#addEditUser_form_password').fill(password);

  await page.locator('#addEditUser_form_confirmPassword').fill(password);
...
async function ensureUserExists(page: Page, username: string): Promise<any> {
...
test('Philips Instance', async ({ page }) => {
  ...
  const data = await ensureUserExists(page, NewUser)

  await page.waitForTimeout(10000);
  return { userExist: data === true ? true : false } as any
